...
{
	if (osek_is_task_activated(&TaskID)) {
		return E_OS_LIMIT;
	}

	if (!osek_task_list_exist(&TaskID)) {
		return E_OS_ID;
	}

	k_thread_resume(TaskID.id);
	return E_OK;
}
...
StatusType TerminateTask(void)
{
	...
	TaskRefType task = osek_zephyr_thread_to_osek(current_thread);
	if (task == NULL) {
		return E_OS_SYS;
	}
...
{
	k_tid_t current_thread = k_current_get();
	TaskRefType current_task = osek_zephyr_thread_to_osek(current_thread);
...
{
	if (TaskID.id == k_current_get()) {
		*State = RUNNING;
		return E_OK;
	}

	uint8_t state = TaskID.id->base.thread_state;
	switch (state) {
	case _THREAD_DUMMY:
...
void RegisterOsekTask(TaskRefType TaskRef) {
	osek_task_list_append(TaskRef);
}